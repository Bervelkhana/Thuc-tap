...
<template>
  <div class="min-h-screen bg-[var(--bg-body)] font-body transition-colors duration-300">
    ...
    <section class="relative flex items-center justify-center bg-white dark:bg-slate-900 overflow-hidden py-32">
      <div class="relative text-center max-w-3xl mx-auto px-8 space-y-12">
        <!-- Main Heading -->
        <h2 class="text-6xl md:text-7xl font-light text-gray-900 dark:text-white leading-tight tracking-tight">
        ...
        <button 
          ...
          class="inline-flex items-center justify-center px-10 py-4 bg-black dark:bg-white text-white dark:text-gray-900 font-medium rounded-lg overflow-hidden group hover:bg-gray-900 dark:hover:bg-gray-100 transition-all duration-300 shadow-sm hover:shadow-xl"
        >
    ...
    <section class="max-w-6xl mx-auto px-8 py-32 space-y-16">
      <div class="space-y-6">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-widest">Danh mục</h3>
        <div class="w-16 h-px bg-gray-900 dark:bg-white"></div>
      </div>
      
      <!-- Loading State -->
      <div v-if="loading.categories" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-8">
        <div v-for="i in 7" :key="i" class="h-32 bg-gray-100 dark:bg-slate-800 rounded-lg animate-pulse"></div>
      </div>
      
      <!-- Error State -->
      <div v-else-if="error.categories" class="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-8 text-center">
        <p class="text-gray-600 dark:text-gray-300">Không thể tải danh mục. Vui lòng thử lại.</p>
      </div>
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-8">
        <router-link
          ...
          :key="c.id"
          :to="`/category/${c.id}`"
          class="group flex flex-col items-center gap-5 p-8 bg-white dark:bg-slate-800 rounded-lg hover:bg-black dark:hover:bg-white hover:text-white dark:hover:text-gray-900 transition-all duration-300 border border-gray-100 dark:border-slate-700 hover:border-black dark:hover:border-white cursor-pointer"
        >
          <span class="text-4xl group-hover:scale-110 transition-transform duration-300">{{ c.icon }}</span>
          <span class="text-xs font-medium text-gray-700 dark:text-gray-300 group-hover:text-white dark:group-hover:text-gray-900 text-center">{{ c.name }}</span>
        </router-link>
      </div>
    </section>
    ...
    <section class="max-w-6xl mx-auto px-8 py-32 space-y-16">
      <div class="space-y-6">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-widest">🔥 Sản phẩm giảm giá</h3>
        <div class="w-16 h-px bg-gray-900 dark:bg-white"></div>
      </div>
      
      <!-- Loading State -->
      <div v-if="loading.sales" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="i in 6" :key="i" class="bg-white dark:bg-slate-800 rounded-lg border border-gray-100 dark:border-slate-700 overflow-hidden">
          <div class="h-48 bg-gray-100 dark:bg-slate-700 animate-pulse"></div>
          <div class="p-8 space-y-4">
            <div class="h-4 bg-gray-100 dark:bg-slate-700 rounded animate-pulse w-1/3"></div>
            <div class="h-4 bg-gray-100 dark:bg-slate-700 rounded animate-pulse w-2/3"></div>
            <div class="h-6 bg-gray-100 dark:bg-slate-700 rounded animate-pulse w-1/2"></div>
      ...
      <!-- Error State -->
      <div v-else-if="error.sales" class="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-8 text-center">
        <p class="text-gray-600 dark:text-gray-300">Không thể tải sản phẩm giảm giá. Vui lòng thử lại.</p>
      </div>
      
      <!-- Empty State -->
      <div v-else-if="saleProducts.length === 0" class="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-12 text-center">
        <p class="text-gray-600 dark:text-gray-300">Hiện tại không có sản phẩm giảm giá.</p>
      </div>
    ...
    <section class="max-w-6xl mx-auto px-8 py-32 space-y-16">
      <div class="space-y-6">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-widest">✨ Sản phẩm mới</h3>
        <div class="w-16 h-px bg-gray-900 dark:bg-white"></div>
      </div>
      ...
      <!-- Error State -->
      <div v-else-if="error.newest" class="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-8 text-center">
        <p class="text-gray-600 dark:text-gray-300">Không thể tải sản phẩm mới. Vui lòng thử lại.</p>
      </div>
      
      <!-- Empty State -->
      <div v-else-if="newestProducts.length === 0" class="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-12 text-center">
        <p class="text-gray-600 dark:text-gray-300">Hiện tại không có sản phẩm mới.</p>
      </div>
    ...
    <section class="max-w-6xl mx-auto px-8 py-32 space-y-16">
      <div class="space-y-6">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-widest">⚡ Flash Sale</h3>
        <div class="w-16 h-px bg-gray-900 dark:bg-white"></div>
      </div>
      
      <!-- Loading State -->
      <div v-if="loading.products" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
        <div v-for="i in 5" :key="i" class="bg-white dark:bg-slate-800 rounded-lg border border-gray-100 dark:border-slate-700 overflow-hidden">
          <div class="h-48 bg-gray-100 dark:bg-slate-700 animate-pulse"></div>
          <div class="p-8 space-y-4">
            <div class="h-4 bg-gray-100 dark:bg-slate-700 rounded animate-pulse w-1/3"></div>
            <div class="h-4 bg-gray-100 dark:bg-slate-700 rounded animate-pulse w-2/3"></div>
      ...
      <!-- Error State -->
      <div v-else-if="error.products" class="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-8 text-center">
        <p class="text-gray-600 dark:text-gray-300">Không thể tải sản phẩm Flash Sale. Vui lòng thử lại.</p>
      </div>
      
      <!-- Empty State -->
      <div v-else-if="flashSale.length === 0" class="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-12 text-center">
        <p class="text-gray-600 dark:text-gray-300">Hiện tại không có sản phẩm Flash Sale.</p>
      </div>
        ...
        >
           ...
           <div class="p-8 space-y-4">
             <div class="flex items-center justify-between">
              <span class="bg-black dark:bg-white text-white dark:text-gray-900 text-xs font-bold px-3 py-1.5 rounded">
                -{{ discount(item) }}%
              </span>
            </div>
    ...
    <!-- ===== FOOTER - Minimal ===== -->
    <footer class="bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-700 py-16">
      <div class="max-w-6xl mx-auto px-8 text-center">
        <p class="text-sm text-gray-600 dark:text-gray-300">© 2026 TechGear. All rights reserved.</p>
      </div>
    </footer>
